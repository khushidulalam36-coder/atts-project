...
if (!TOKEN) {
  console.error('❌ VERCEL_BLOB_READ_WRITE_TOKEN environment variable is not set!');
} else {
  console.log('✅ Blob token found (length:', TOKEN.length, 'chars)');
}

function getBlobOptions(access = 'public') {
  if (!TOKEN) throw new Error('Blob token is not configured');
  return { access, token: TOKEN, cacheControl: 'public, max-age=60' };
}
