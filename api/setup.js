...
// ====================== Node.js to Fetch API wrapper (fixed) ======================
...
async function apiHandler(req) {
  ...
  try {
    ...
    if (path === '/init-db' && req.method === 'POST') {
      ...
      // Check if tables already exist (optional safety)
      ...
      return json({ message: 'Database initialized successfully. Please note the temporary admin password shown in server logs.' });
    }
    ...
    if (path === '/admin/chapter' && req.method === 'POST') {
      ...
      if (!adminUser) return errorJson('Forbidden', 403);
      const body = await req.json();
      const { course_id, title, order_index, content_text, image_url, video_url, passing_score } = body;
    ...
    if (path.match(/^\/admin\/chapter\/(\d+)$/) && req.method === 'PUT') {
      ...
      const body = await req.json();
      const { title, order_index, content_text, image_url, video_url, passing_score, is_active } = body;
    ...
    if (path.match(/^\/admin\/chapter\/(\d+)\/question$/) && req.method === 'POST') {
      ...
      const chapterId = parseInt(path.split('/')[3]);
      const body = await req.json();
      const { question, options, correct_index, explanation, order_index } = body;
    ...
    // Admin Simulate Days (FIXED: removed admin_secret, now only admin auth)
    ...
    // Admin content management with listing/editing of quizzes and videos
    ...
    if (path.match(/^\/admin\/content\/(lesson|quiz|video)\/(\d+)$/) && req.method === 'DELETE') {
      ...
      const allowedTables = ['lessons', 'quizzes', 'video_library'];
      if (!allowedTables.includes(tableMap[type])) return errorJson('Invalid table', 400);
    ...
    // ---------------- AUTH REQUIRED ROUTES (user authenticated) ----------------
